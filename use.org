...
An adequately complete LaTeX installation is needed to compile the latex
documents.
...
tests for [[https://github.com/xraypy/feff85exafs][feff85exafs]],
including a mustache template for =feff.inp=, a json file controlling
how the template is filled in, χ(k) data, a python script defining a
fitting model using Larch, and a Feff6 input file.
...
In the paper that introduced Feff6 ("Multiple-scattering calculations of
x-ray-absorption spectra", S. I. Zabinsky, J. J. Rehr, A. Ankudinov, R.
C. Albers, and M. J. Eller, Phys. Rev. *B*52, p. 2995,
[[http://dx.doi.org/10.1103/PhysRevB.52.2995][doi:10.1103/PhysRevB.52.2995]]),
...
The formatting of the table is controlled by the =tableformat= attribute
and can be any of the
[[https://pypi.python.org/pypi/tabulate#table-format][strings specified
here]]. By default, the =pipe= format is used. =latex= is handled
specially by the =table()= method.
...
2. =Copper.mustache=: a feff.inp for running Feff8 with certain values
   replaced by [[https://github.com/mustache/mustache][mustache]] tokens
...
The file =results.tex= gathers up the output of all the stuff above for
conversion to a nice PDF file. Eventually I decided I wanted to make a
GitHub Pages website instead. The content of =results.tex= was the
starting point for the current =README.md= file.
...
If you add a new material, you should edit the =fefftest.py= plugin file
to add the name of your new test to the list at line 43.
