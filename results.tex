...
of 0.78.  The simple fiting model included a S$_0^2$ parameter
(\texttt{amp}), an energy shift (\texttt{enot}), and a volumetric
lattice expansion coefficient (\texttt{alpha}).  
...
This is one of my standard teaching example.  It's good for teaching
...
$sigma^2$ parameter (\texttt{ss} and \texttt{ss2}).  The thrid shell
...
improvment to the fit, so the \texttt{ss2} parameter is used for its
$\sigma^2$.  Finally a $\sigma^2$ parameter is floated for the Fe
shell.
...
al. (and what a sordid all \textit{that} was) proposed that
...
concept is that, this would approximate any errors in the scattering
phase shifts.
...
A variety of collinear multiple scattering paths at the distance of
the third shell Zr scatterer are included in the fit.  The energy
shifts are parameterized as described in the paper.
...
\includegraphics[width=0.2\linewidth]{bromoadamantane/bromoadamantane.png}
...
0.9 for each potential was rquired to even get \textsc{feff}8.5 to run
to completion.

\includegraphics[width=0.2\linewidth]{uranyl/uranyl.png}
