...
#+BEGIN_QUOTE
  Order of the approximation used in module =GENFMT=.
  =feff= uses order 2 by default, which is correct to terms of
  order 1/(pR)^2 and corresponds to 6x6 scattering matrices
  in the Rehr-Albers formalism.  Single scattering is calculated
  exactly to this order.  The 6x6 approximation is accurate to
  ...
  M4 shells and higher shells may require increased iorder for
  ...
  module =GENFMT=.  To do so, follow the instructions in the
  feff source code in subroutine
  [[https://github.com/xraypy/feff85exafs/blob/master/src/GENFMT/setlam.f][=setlam=]].  The key =iord= is passed to
  =setlam= for processing.  You may need to change
  [[https://github.com/xraypy/feff85exafs/blob/master/src/HEADERS/dim.h#L37][the code parameter =lamtot=]] if you want to do higher order
  calculations.  For details of the algorithm used by =GENFMT=,
  see [[http://dx.doi.org/10.1103/PhysRevB.41.8139][the paper by J.J. Rehr and R.C. Albers]]. For the M4 and
...
Changes to the =iord= parameter should only effect multiple
...
shell fits with different values if =iord= does, in fact,
result in identical fit results.  For example, here are the results
for first shell fits to FeS2:
...
1. A couple of the materials pretty much as one might expect.  NiO,
   Bromoadamantane, and BaZrO3 show a significant drop in chi-reduced
   between =iord= of 1 and 2, while not showing a statistically
   significant change in any of the fitting parameters.
...
4. Perhaps this exercise could be used to approximate the
